...
function fillDataList(msg,pageindex){
    var html = "";
    if (msg != null && msg != "") {
        html = "<tr><th>标题</th><th>摘要</th><th>讲师</th><<th>来源</th><th>课程视频</th><th>课程封面</th><th>创建时间</th><th>修改 | 删除</th></tr>";
        ...
        $.each(msg, function (i, n) {
            ...
            html += "<td>" + (n.teacher != null ? n.teacher : "--") + "</td>";
            //html += "<td>" + n.course_type + "</td>";
            html += "<td>" + (n.source != null? n.source:'') + "</td>";
            html += "<td>" + (n.video != null? n.video:'') + "</td>";
            html += "<td>" + (n.imagepath != null? "<img src='"+n.imagepath+"' style='height:24px;'/>":'') + "</td>";
...
function Add(id){
    ...
    var teacher = $.trim($("input[name=teacher]").val());
    ...
    //封面
    var imageid="";
    if($("input[name=imageid]").length>0&&$("input[name=imageid]").val()!=""){
        imageid = $.trim($("input[name=imageid]").val());
    }
    var description = $.trim($("textarea[name=description]").val());
    if(validateForm(title)){
        var postdata={title:title,teacher:teacher,source:source,video:video,fileid:fileid,imageid:imageid,description:description};
        if(id!=null&&id!=""){
            postdata['id']=id;
        }
...
//图片
function initCourseImage(){
    //相册弹框
     var param_uploader2= {
        headers: {'X-Requested-With': 'XMLHttpRequest', 'X-CSRFToken': $.cookie('csrftoken'),'Accept':'application/json,text/javascript'},
        runtimes: 'html5,flash,silverlight,html4',
        browse_button: 'pick_files', // you can pass in id...
        container: document.getElementById('upload_img_container'), // ... or DOM Element itself
        url:urls.url_course_file,
        flash_swf_url : '/static/js/plupload/Moxie.swf',
        filters: {
            max_file_size: '100mb',
        ...
        init: {
            ...
            FilesAdded: function (up, files) {
                ...
                plupload.each(files, function (file) {
                    ...
                    html_init_img += "<span class=\"image-persent\">0%</span><span>"+plupload.formatSize(file.size)+"</span><a href=\"javascript:;\" onclick=\"deleteUploadFile('" + file.id + "',1);\"><i class=\"icon-trash\"></i> 删除</a>";
                    html_init_img += "</div></div>";
                    document.getElementById('imgList').innerHTML +=html_init_img;
                });
            ...
            FileUploaded: function (up, file, info) {
                ...
                if (info.response != null&&info.response!="") {
                    ...
                    if (msg.result) {
                        var fileid = msg.data;
                        document.getElementById("imgid").innerHTML = "<input name=\"imageid\" type=\"hidden\" value=\"" + fileid + "\" />"
                    }
                }
            },
            Error: function (up, err) {
    ...
    //文件上传[相册]
    if(document.getElementById('pick_files')!=null){
